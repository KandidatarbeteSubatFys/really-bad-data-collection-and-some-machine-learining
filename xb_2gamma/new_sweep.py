...
def hidden_laysers(input, dim, nr, relu=True, dtype=tf.float32):
    ...
    b = tf.Variable(tf.constant(0.1),[dim], dtype=dtype)
    if  nr == 1:
        if relu:
...
def def_fc_layers(input, start_nodes, end_nodes, hidden_nodes, nr_hidden_laysers, relu=True, dtyp=tf.float32):
    ...
    b_start = tf.Variable(tf.constant(0.1),[hidden_nodes])
    W_end = tf.Variable(tf.truncated_normal([hidden_nodes, end_nodes], stddev=0.1), dtype=dtyp)
    b_end = tf.Variable(tf.constant(0.1),[end_nodes])
    if relu:
...
def get_nr_parameters(nodes, layers):
    # Mycket dålig function fungerar bara för just detta nätverk, problemet var att den såg alla tidigare definerade variabler och lade till parametrarna av dessa. 
    #return np.sum([np.prod(v.get_shape().as_list()) for v in tf.trainable_variables(scope='fully_conected')])
    return 162*nodes+nodes+ (layers - 1)*(nodes*nodes + nodes)+ 2*nodes + 2


def main(file_name_x, file_name_y, dep_file_name, nr_nodes_hidden, nr_hidden_laysers, size_of_data, nr_iterations):
    ...
    if int(0.01*len(x_batch))+size_of_data > len(x_batch):
        raise ValueError('givna datastorleken större än tillgänglig data')
    else:
        x_batch_train = x_batch[int(0.01*len(x_batch)):int(0.01*len(x_batch))+size_of_data]
    y_batch = read_data(file_name_y)
    y_batch_eval = y_batch[0:int(0.01 * len(x_batch))]
    if int(0.01*len(x_batch))+size_of_data > len(y_batch):
        raise ValueError('givna datastorleken större än tillgänglig data')
    else:
        y_batch_train = y_batch[int(0.01 * len(x_batch)):int(0.01*len(x_batch))+size_of_data]

    print('Start training')
    print('Hidden layers: ', nr_hidden_laysers, ' Nodes per layer: ', nr_nodes_hidden, ' nr param: ', get_nr_parameters(nr_nodes_hidden, nr_hidden_laysers), ' #exponeringar: ', int(nr_iterations/size_of_data)) 
    start = t.time()
    i = 0
    diff = 10
    tol = 0.000001
    while m.fabs(diff) > tol and i < nr_iterations:
    ...
    dep = [i[0] for i in read_data(dep_file_name)[0:int(0.01*len(x_batch))]]
                ...
                with open('err.txt', 'w') as f4:
                    for laysers in hidden_layers:
                        for nodes in hidden_nodes:
                            for exp in antal_exp:
                                nr_iter=20* get_nr_parameters(nodes,laysers)
                                datasize = int(nr_iter/exp)
                                loss, mean, std, err, out, out_corr, loss_list, loss_training_list, trainingtime, iterations, dep, sum_out = \
                                                                                                                                             main('../../xb_data_2gamma_isotropic_0.1-10_1miljon.txt', '../../gunTVals_0.1-10.txt', '../../sum_of_dep_energies.txt', nodes, laysers, datasize, nr_iter)

                                f1.write(str(n) + ' ' + str(laysers) + ' ' + str(nodes) + ' ' + str(loss) + ' ' + str(mean) + ' ' + str(std) + ' '+ str(trainingtime) + ' ' + str(iterations) + ' ' +  str(exp) + str( ' \n'))
                                loss_str = ''
                                loss_train_str = ''
                                ...
                                ax[0, 0].set(ylabel='Loss function', xlabel='Iteration')
                                ...
                                ax[1, 1].set(xlabel='Total deposited energy (MeV)', ylabel='Predicted tot energy (MeV)')
                                fig.suptitle('Hidden layers: ' + str(laysers) + ', nodes per layer: ' + str(nodes) + ', iterations: ' + str(iterations) + ', #exposures: ' + str(exp))
                                n = n +1
                                name = str(n) + '.' + str(laysers) + '.' + str(nodes) + '.' + str(exp) + '.png'
                                plt.savefig(name)
                                plt.clf()
