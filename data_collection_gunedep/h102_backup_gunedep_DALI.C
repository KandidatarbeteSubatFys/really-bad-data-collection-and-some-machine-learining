...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   FILE * dataFile;
   const char * inDatatxt = "DALI_det_data.txt";
   dataFile = fopen(inDatatxt,"w"); // detector output data

   FILE * gunTFile;
   const char * correctGunDatatxt = "DALI_gun_vals.txt";
   gunTFile = fopen(correctGunDatatxt,"w"); // "correct" gun data (energies, cos(theta), etc.)

   FILE * depEFile;
   const char * totDepEtxt = "DALI_sum_of_dep_energies.txt";
   depEFile = fopen(totDepEtxt,"w"); // sum of DALIe for each event
   ...
   for (Long64_t jentry=0; jentry < nentries; jentry++) {
	...
	goodEvent = gunedepDALI[i]/gunT[i] > 0.9;
	if (!goodEvent) break;
      }
